...
from posthog.schema import VisualizationArtifactContent, VisualizationArtifactMessage
    ...
    async def _arun_impl(self, kind: ReadDataAdminAccessKind | ReadDataKind) -> tuple[str, None]:
        match kind:
            ...
            case "artifacts":
                ...
                state_artifacts = cast(
                    list[VisualizationArtifactMessage],
                    await self._context_manager.artifacts.aenrich_messages(messages, artifacts_only=True),
                )
                subagent_artifacts = await self._context_manager.artifacts.aget_subagent_artifacts(messages)

                return (
                    f"Generated by you:\n{self._format_artifacts(state_artifacts)}\n\n"
                    f"Generated by subagents:\n{self._format_artifacts(subagent_artifacts)}",
                    None,
                )

    def _format_artifacts(self, artifacts: list[AgentArtifact] | list[VisualizationArtifactMessage]) -> str:
        formatted_artifacts = []
        for artifact in artifacts:
            if isinstance(artifact, AgentArtifact) and artifact.type == AgentArtifact.Type.VISUALIZATION:
                try:
                    content = VisualizationArtifactContent.model_validate(artifact.data)
                except ValidationError as e:
                    capture_exception(e)
                    continue
            elif isinstance(artifact, VisualizationArtifactMessage):
